...
struct SupportsAudioSourceByRef : std::false_type {};

template <typename Factory>
struct SupportsAudioSourceByRef<
    Factory,
    std::void_t<decltype(std::declval<Factory*>()->CreateAudioSource(
        std::declval<const webrtc::AudioOptions&>()))>> : std::true_type {};

template <typename Factory, typename = void>
struct SupportsAudioSourceByPtr : std::false_type {};

template <typename Factory>
struct SupportsAudioSourceByPtr<
    Factory, std::void_t<decltype(std::declval<Factory*>()->CreateAudioSource(
                 std::declval<webrtc::AudioOptions*>()))>> : std::true_type {};

template <typename Factory>
webrtc::scoped_refptr<webrtc::AudioSourceInterface> CreateAudioSourceCompat(
    Factory* factory, webrtc::AudioOptions* options) {
  if constexpr (SupportsAudioSourceByRef<Factory>::value) {
    return factory->CreateAudioSource(*options);
  } else if constexpr (SupportsAudioSourceByPtr<Factory>::value) {
    return factory->CreateAudioSource(options);
  } else {
    return nullptr;
  }
...
scoped_refptr<RTCAudioSource> RTCPeerConnectionFactoryImpl::CreateAudioSource(
  ...
  if (source_type == RTCAudioSource::SourceType::kCustom) {
    webrtc::scoped_refptr<libwebrtc::LocalAudioSource> custom_source =
        libwebrtc::LocalAudioSource::Create(&rtc_options);
    return scoped_refptr<RTCAudioSourceImpl>(new RefCountedObject<
                                             RTCAudioSourceImpl>(
        custom_source, custom_source, source_type));
  }

  webrtc::scoped_refptr<webrtc::AudioSourceInterface> default_source =
      CreateAudioSourceCompat(rtc_peerconnection_factory_.get(), &rtc_options);
  if (!default_source.get()) {
    // Fallback keeps the API usable even if upstream signatures drift.
    webrtc::scoped_refptr<libwebrtc::LocalAudioSource> fallback_source =
        libwebrtc::LocalAudioSource::Create(&rtc_options);
    return scoped_refptr<RTCAudioSourceImpl>(new RefCountedObject<
                                             RTCAudioSourceImpl>(
        fallback_source, fallback_source, source_type));
  }

  return scoped_refptr<RTCAudioSourceImpl>(
      new RefCountedObject<RTCAudioSourceImpl>(default_source, nullptr,
                                               source_type));
}
