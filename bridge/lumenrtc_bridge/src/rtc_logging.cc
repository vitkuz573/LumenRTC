...
namespace lumenrtc_bridge {
  ...
  void LumenRtcBridgeRuntimeLogging::setMinDebugLogLevel(RTCLoggingSeverity severity) {
    rtc::LogMessage::LogToDebug(getNativeLoggingSeverity(severity));
  }

  void LumenRtcBridgeRuntimeLogging::setLogSink(RTCLoggingSeverity severity, RTCCallbackLoggerMessageHandler callbackHandler) {
    removeLogSink();
  ...
  void LumenRtcBridgeRuntimeLogging::removeLogSink() {
    if(log_sink)
