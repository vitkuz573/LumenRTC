...
class LumenRtcBridgeRuntimeConan(ConanFile):
    name = 'lumenrtc_bridge'
    version = '0.0.1'
    ...
    webrtc_release = 'branch-heads/7151'
    webrtc_dir = 'src'
...
""".format(self.webrtc_dir, self.webrtc_release)
    ...
    def _setup_webrtc(self):
        ...
        with chdir(self, 'src'):
            ...
            with fileinput.FileInput('BUILD.gn', inplace=True) as file:
                for line in file:
                    print(line.replace("deps = [ \":webrtc\" ]", "deps = [ \":webrtc\", \"//lumenrtc_bridge\" ]"), end='')
    ...
    def _gn_args(self):
        ...
        if self.settings.os == 'Windows':
            ...
            args.append('lumenrtc_bridge_desktop_capture=true')
        else:
    ...
    def _build_webrtc(self):
        with chdir(self, 'src'):
            ...
            self.run("ninja -C out lumenrtc_bridge")
    ...
    def package(self):
        sources_folder = os.path.join(self.build_folder, 'src', 'lumenrtc_bridge')
        ...
        copy(self, 'lumenrtc_bridge.dll.lib', src=os.path.join(self.build_folder, 'src', 'out'), dst=os.path.join(self.package_folder, 'lib'), keep_path=False)
        copy(self, 'lumenrtc_bridge.dll', src=os.path.join(self.build_folder, 'src', 'out'), dst=os.path.join(self.package_folder, 'lib'), keep_path=False)
        copy(self, 'lumenrtc_bridge.so', src=os.path.join(self.build_folder, 'src', 'out'), dst=os.path.join(self.package_folder, 'lib'), keep_path=False)
    ...
    def package_info(self):
        ...
        if self.settings.os == 'Windows':
            self.cpp_info.libs = ["lumenrtc_bridge.dll"]
        else:
            self.cpp_info.libs = ["lumenrtc_bridge.so"]
