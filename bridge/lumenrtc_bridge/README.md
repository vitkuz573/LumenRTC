# lumenrtc_bridge-{windows,linux}-{amd64,x86,armv7,arm64}.{dll,so}
...
```bash
mkdir lumenrtc_bridge_build
cd lumenrtc_bridge_build
```
...
Copy this folder into your WebRTC checkout as `src/lumenrtc_bridge` (or use
LumenRTC `scripts/setup.sh` / `scripts/setup.ps1`, which performs this
automatically).

Ensure `src/BUILD.gn` includes `//lumenrtc_bridge` in `group("default")`:
...
cd src
gn gen out-debug/Windows-x64 --args="target_os=\"win\" target_cpu=\"x64\" is_component_build=false is_clang=true is_debug=true rtc_use_h264=true ffmpeg_branding=\"Chrome\" rtc_include_tests=false rtc_build_examples=false lumenrtc_bridge_desktop_capture=true" --ide=vs2022
ninja -C out-debug/Windows-x64 lumenrtc_bridge
```
...
ninja -C out-debug/Linux-x64 lumenrtc_bridge
```
